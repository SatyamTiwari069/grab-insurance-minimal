...
function classifyIntent(deal) {
    ...
    let confidence = 0.62;
    if (subcatKey)
        confidence += 0.15; // Subcategory match bonus
    if (deal.dealValue > 10000)
        confidence += 0.05; // High-value deal bonus
    if (deal.dealValue > 50000)
        confidence += 0.03; // Very high-value bonus
    if (deal.userHistory && deal.userHistory.length > 0)
        confidence += 0.08; // Returning user
