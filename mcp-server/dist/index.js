...
const INSURANCE_CATALOG = [
    {
        ...
        categories: ["Travel"],
        icon: "🛫",
        description: "Full refund if your trip gets cancelled due to emergencies, illness, or visa rejection. Covers flights, hotels, and packages.",
    ...
    },
    {
        ...
        categories: ["Travel"],
        icon: "🏥",
        description: "Covers medical emergencies, hospital stays, and evacuation during travel. Includes COVID-19 cover.",
    ...
    },
    {
        ...
        icon: "🔧",
        description: "Extends manufacturer warranty by 2 years. Covers hardware defects, malfunctions, and electrical surges.",
    ...
    },
    {
        ...
        categories: ["Electronics"],
        icon: "📱",
        description: "Unlimited screen repairs for accidental drops and cracks. Zero deductible, doorstep service.",
    ...
    },
    {
        ...
        icon: "🛡️",
        description: "24/7 accident coverage. Hospitalization, disability, and accidental death benefit included.",
    ...
    },
    {
        ...
        categories: ["Health"],
        icon: "💊",
        description: "Covers doctor consultations, diagnostics, medicine, and OPD treatments. No waiting period.",
    ...
    },
    {
        ...
        icon: "↩️",
        description: "Full refund within 30 days, no questions asked. Includes free pickup and return shipping.",
    ...
    },
    {
        ...
        icon: "🔒",
        description: "Covers price drops within 30 days and accidental damage from day one. Instant claims.",
...
function classifyIntent(deal) {
    ...
    const subcatKey = Object.keys(rules).find((k) => k !== "default" && deal.subcategory.toLowerCase().includes(k.toLowerCase()));
    ...
    function scoreProduct(product) {
        ...
        if (subcatKey)
            score += 0.15; // Subcategory rule matched — strong signal
        ...
        if (deal.dealValue > 10000)
            score += 0.05;
        if (deal.dealValue > 50000)
            score += 0.03;
        // Risk tier — high-risk deals clearly benefit from cover
        if (deal.riskTier === "high")
            score += 0.05;
        else if (deal.riskTier === "low")
            score -= 0.02;
        // Returning user adds mild signal (more data, not a product-fit signal)
        if (deal.userHistory && deal.userHistory.length > 2)
            score += 0.04;
        else if (deal.userHistory && deal.userHistory.length > 0)
            score += 0.02;
...
// ═══════════════════════════════════════════════════════════════
function calculatePremium(dealValue, product, riskTier = "medium") {
    const riskMultipliers = { low: 0.8, medium: 1.0, high: 1.25 };
...
function generateCopy(deal, product, premium) {
...
function resolveMultiCart(deals) {
    const byCategory = {};
    ...
    const sorted = categories.sort((a, b) => CATEGORY_PRIORITY.indexOf(a) - CATEGORY_PRIORITY.indexOf(b));
...
function createHttpServer() {
    ...
    app.post("/api/multi-cart/resolve", (req, res) => {
        try {
    ...
    app.post("/api/cart/add", (req, res) => {
        ...
        if (!cartItems.has(sessionId))
            cartItems.set(sessionId, []);
    ...
    app.delete("/api/cart/:sessionId/:index", (req, res) => {
        const items = cartItems.get(req.params.sessionId) || [];
...
const MCP_TOOLS = [
    {
        name: "classify_deal",
        description: "Classify a deal and get top 2 insurance product recommendations with confidence scores. Takes deal info (merchant, category, subcategory, value, risk) and returns the best matching insurance products with dynamic pricing and personalized copy.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Generate hyper-personalized insurance offer copy for a deal. Returns 3 A/B variants: Direct (A), Emotional (B), Social proof (C).",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "List all available insurance products in the catalog, optionally filtered by category.",
        inputSchema: {
            type: "object",
            properties: {
                category: {
    ...
    },
    {
        ...
        description: "Resolve insurance recommendations when a user has deals from multiple categories (e.g., Myntra + MakeMyTrip). Determines priority and which insurance products to show.",
        inputSchema: {
            type: "object",
            properties: {
                deals: {
    ...
    {
        name: "get_analytics",
        description: "Get A/B testing analytics: conversion rates, variant performance, revenue by category, and top products.",
        inputSchema: {
            type: "object",
            properties: {},
        },
...
async function handleMcpToolCall(name, args) {
    switch (name) {
        ...
        case "get_premium_quote": {
            ...
            if (!product)
                return JSON.stringify({ error: `Product "${args.productId}" not found` });
        ...
        case "get_analytics": {
            const variantStats = { variant_A: 0, variant_B: 0, variant_C: 0 };
            const categoryRevenue = {};
            for (const c of conversions) {
